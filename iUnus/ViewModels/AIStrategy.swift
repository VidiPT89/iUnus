...
enum AIStrategy {
    ...
    static func chooseCard(hand: [Card], topCard: Card, opponentLowestCardCount: Int) -> Card? {
    ...
    static func chooseColor(hand: [Card], excluding played: Card) -> CardColor {
        ...
        return counts.max(by: { $0.value < $1.value })?.key ?? CardColor.playableColors.randomElement()!
    }

    static func shouldCallUnoImmediately() -> Bool {
